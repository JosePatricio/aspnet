...
namespace Sigeor.GestionMgl
{
    ...
    public partial class EorCabeceraMaquinariaWebForm : System.Web.UI.Page
    {
        ...
        public EorCabeceraMaquinariaWebForm()
        {
            try
            {
        ...
        protected void VerReporte(object sender, EventArgs e)
        {
            try
            {
                GestionUtil.VerReporte(this, "EorCabeceraMaquinariaReport", RecuperarFiltros());
            }
            catch (Exception ex)
            {
        ...
        protected void RegistrosVisiblesDropDownList_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                paginaActualTextBox.Text = "1";
                EorCabeceraMaquinariaGridView.PageSize = PageSizeActual;
                CargarDatosPorEstadoCoincidencia();
            }
            catch
            {
        ...
        protected void EorCabeceraMaquinariaGridView_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    var footerGroups = ((DataRowView)(e.Row.DataItem)).Row.ItemArray[e.Row.Cells.Count - 6].ToString();
                    ...
                    else
                    {
                        if (footerGroups.Contains("TOTAL LÍNEA:") || footerGroups.Contains("TOTAL DEPÓSITO:") || footerGroups.Contains("GRAN TOTAL"))
                        {
                            ...
                            e.Row.Cells[e.Row.Cells.Count - 7].ColumnSpan = e.Row.Cells.Count - 6;
                            e.Row.Cells[e.Row.Cells.Count - 7].HorizontalAlign = HorizontalAlign.Right;

                            for (int i = 0; i < e.Row.Cells.Count - 7; i++)
                                e.Row.Cells[i].Visible = false;
                        }
                        else
                        {
                            ...
                            var valorPtiNegociacion = decimal.Parse(((DataRowView)(e.Row.DataItem)).Row.ItemArray[e.Row.Cells.Count - 4].ToString().Replace("$", string.Empty));
                            var valorPtiEstimado = decimal.Parse(((DataRowView)(e.Row.DataItem)).Row.ItemArray[e.Row.Cells.Count - 5].ToString().Replace("$", string.Empty));
        ...
        private void BindingDatosPaginadosPorLista(List<GET_CABECERA_EOR_MAQUINARIA_Result> lista, int totalRegistros)
        {
            try
            {
                var totalPaginas =
               (Math.Round(new decimal(totalRegistros / PageSizeActual), 0, MidpointRounding.AwayFromZero));
                ...
                dataTable.Columns.Add(new DataColumn("NUM_EORMAQ"));
                ...
                dataTable.Columns.Add(new DataColumn("FECHA_EORMAQ"));
                ...
                foreach (var dep in depositos)
                {
                    ...
                    dataRow["NUM_EORMAQ"] = "  DEPÓSITO: " + dep;
                    ...
                    foreach (var lin in lineas)
                    {
                        ...
                        {
                            dataRow = dataTable.NewRow();
                            dataRow["NUM_EORMAQ"] = eor.NUM_EORMAQ;
                            ...
                            dataRow["FECHA_EORMAQ"] = eor.FECHA_EORMAQ != null ? eor.FECHA_EORMAQ.Value.ToString("dd/MM/yyyy") : string.Empty;
                            ...
                            dataRow["PTI_ESTIMADO"] = GestionUtil.ConvertirStringToDivisa(eor.PTI_ESTIMADO);
                            dataRow["PTI_NEGOCIACION"] = GestionUtil.ConvertirStringToDivisa(eor.PTI_NEGOCIACION);
                            ...
                            sumaLineaPtiEstimado += eor.PTI_ESTIMADO;
                            sumaLineaPtiNegociacion += eor.PTI_NEGOCIACION;
                ...
                EorCabeceraMaquinariaGridView.DataSource = dataTable;
                EorCabeceraMaquinariaGridView.DataBind();
            }
            catch (Exception)
            {
        ...
        private void CargarDatosPaginadosPorNavegacion(object sender)
        {
            try
            {
                ...
                if (control != null)
                {
                    ...
                    var result = _clienteReportes.ObtenerEorCabeceraMaquinariaPorFiltrosPaginado(out _totalRegistros, RecuperarFiltros(), PageSizeActual, PageIndexActual);
                    //ReporteEorMaquinariaNegocio.ObtenerEorCabeceraPorFiltrosPaginado(
                    //            RecuperarFiltros(), PageSizeActual, PageIndexActual,
                    //            out _totalRegistros);

                    var lista = !string.IsNullOrEmpty(result) ? Serializador.DeSerializeEntity<List<GET_CABECERA_EOR_MAQUINARIA_Result>>(result) : new List<GET_CABECERA_EOR_MAQUINARIA_Result>();
