...
namespace Sigeor.GestionControl
{
    ...
    public partial class EstimacionTransitoWebForm : System.Web.UI.Page
    {
        ...
        private string RecuperarFiltros()
        {
            ...
            try
            {
                ...
                if (listaContenedores.Any())
                {
                    ...
                    {
                        filtros.DescripcionCuatro += "'" + item.Value + "',";
                    }
                    filtros.DescripcionCuatro = filtros.DescripcionCuatro.Substring(0, filtros.DescripcionCuatro.Length - 1);
                }
        ...
        public EstimacionTransitoWebForm()
        {
            try
            {
        ...
        protected void RegistrosVisiblesDropDownList_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                paginaActualTextBox.Text = "1";
                EorCabeceraTransitoGridView.PageSize = PageSizeActual;
                CargarDatosPorEstadoCoincidencia();
            }
            catch
            {
        ...
        protected void EorCabeceraTransitoGridView_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
        ...
        private void BindingDatosPaginadosPorLista(List<GET_CABECERA_EOR_TRANSITO_Result> lista, int totalRegistros)
        {
            try
            {
                var totalPaginas =
               (Math.Round(new decimal(totalRegistros / PageSizeActual), 0, MidpointRounding.AwayFromZero));
                ...
                dataTable.Columns.Add(new DataColumn("NUM_EORTRANSITO"));
                ...
                dataTable.Columns.Add(new DataColumn("FECHA_REPARACIONEST"));
                dataTable.Columns.Add(new DataColumn("ESTADOEST"));
                dataTable.Columns.Add(new DataColumn("FECHA_REPARACIONMAQ"));
                dataTable.Columns.Add(new DataColumn("ESTADOMAQ"));
                ...
                foreach (var dep in depositos)
                {
                    ...
                    dataRow["NUM_EORTRANSITO"] = "  DEPÓSITO: " + dep;
                    ...
                    foreach (var lin in lineas)
                    {
                        ...
                        if (listaFiltradaEor.Any())
                        {
                            ...
                            dataRow = dataTable.NewRow();
                            dataRow["NUM_EORTRANSITO"] = "   LÍNEA: " + lin;
                            dataTable.Rows.Add(dataRow);
                        ...
                        {
                            dataRow = dataTable.NewRow();
                            dataRow["NUM_EORTRANSITO"] = eor.NUM_EORTRANSITO;
                            dataRow["NUM_CONTAINER"] = eor.NUM_CONTAINER;
                            dataRow["COD_TIPCONT"] = eor.COD_TIPCONT;
                            dataRow["ISO_CODE"] = eor.ISO_CODE;
                            dataRow["NOM_LINEA"] = eor.NOM_LINEA;
                            dataRow["CIUDAD"] = eor.CIUDAD;
                            dataRow["FECHA_EOR"] = eor.FECHA_EOR != null ? eor.FECHA_EOR.Value.ToShortDateString() : string.Empty;                            
                            dataRow["FECHA_REPARACIONEST"] = eor.FECHA_FINREPARAEST != null ? eor.FECHA_FINREPARAEST.Value.ToShortDateString() : string.Empty;
                            dataRow["ESTADOEST"] = eor.ESTADOEST;
                            dataRow["FECHA_REPARACIONMAQ"] = eor.FECHA_FINREPARAMAQ != null ? eor.FECHA_FINREPARAMAQ.Value.ToShortDateString() : string.Empty;
                            dataRow["ESTADOMAQ"] = eor.ESTADOMAQ;
                        ...
                        {
                            dataRow = dataTable.NewRow();
                            dataRow["ESTADOMAQ"] = " TOTAL LÍNEA: " + lin.ToUpper();
                    ...
                    dataRow = dataTable.NewRow();
                    dataRow["ESTADOMAQ"] = " TOTAL DEPÓSITO: " + dep.ToUpper();
                ...
                EorCabeceraTransitoGridView.DataSource = dataTable;
                EorCabeceraTransitoGridView.DataBind();
        ...
        private void CargarDatosPaginadosPorNavegacion(object sender)
        {
            try
            {
                ...
                if (control != null)
                {
                    ...
                    var result = _clienteReportes.ObtenerEorCabeceraTransitoPorFiltrosPaginado(out _totalRegistros, RecuperarFiltros(), PageSizeActual, PageIndexActual);

                    //var result =
                    //        ReporteEorTransitoNegocio.ObtenerEorCabeceraPorFiltrosPaginado(
                    //            RecuperarFiltros(), PageSizeActual, PageIndexActual,
                    //            out _totalRegistros);

                    var lista = !string.IsNullOrEmpty(result) ? Serializador.DeSerializeEntity<List<GET_CABECERA_EOR_TRANSITO_Result>>(result) : new List<GET_CABECERA_EOR_TRANSITO_Result>();
